<?php  //final project:  treehouse.php
  require_once 'header.php';
  
  
  if (!$loggedin) die();

     $result = queryMySQL("SELECT * FROM entries INNER JOIN friends ON entries.entry_user=friends.friend WHERE (friends.user='$user' OR entries.entry_user='$user') 
							AND entries.entry_destination='treehouse' GROUP BY entries.entry_num ORDER BY entries.entry_date"); 
     //$result = queryMySQL("SELECT * FROM entries WHERE entry_user='$user' AND entry_destination='treehouse'");
		//entries.entry_num, entries.edit_date, entries.entry_text, entries.entry_date, entries.entry_user, friends.friend, friends.user
...
_END;
	...
	echo "<form method='post' action='treehouse.php'>";
	...
	for ($j = 0 ; $j < $rows ; ++$j)
	{
		...
		$destination = "treehouse.php";
		echo "<div class='treehouse_date'>" . $row['entry_date'];
		...
		echo "</div><div id='edit$entry_num'>" . $entry_text . "</div><br>";
